...
static void
StereoCalib(const vector<string> &imagelist, Size boardSize, float squareSize, bool displayCorners = false,
    ...
    imagePoints[0].resize(nimages);//存放left的角点2D坐标
    imagePoints[1].resize(nimages);//存放right的角点2D坐标
...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    for (i = 0; i < nimages; i++) {
        int npt = (int) imagePoints[0][i].size();//一张图的角点总数
        Mat imgpt[2];
        for (k = 0; k < 2; k++) {
            imgpt[k] = Mat(imagePoints[k][i]);
            undistortPoints(imgpt[k], imgpt[k], cameraMatrix[k], distCoeffs[k], Mat(), cameraMatrix[k]);
            //根据基础矩阵和当前图片的点，获取在另一张图的对应极线 lines[k]=Fx_l
    ...
    // save EandF
    fs.open("EandF.yml", FileStorage::WRITE);
    if (fs.isOpened()) {
        fs << "E" << E << "F" << F;
        fs.release();
    } else
        cout << "Error: can not save the EandF parameters\n";
